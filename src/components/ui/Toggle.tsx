...
const toggleVariants = cva(
  "inline-flex items-center justify-center rounded-md text-sm font-medium transition-colors disabled:pointer-events-none disabled:opacity-50 data-[state=on]:text-white focus-visible:outline-none",
  {
    variants: {
      ...
      tone: {
        tan: [cls.dataOnBgToggleActive, cls.hoverBgToggleHover, cls.borderToggleActive],
        slate: [cls.dataOnBgRepotActive, cls.hoverBgRepotHover, cls.borderRepotActive],
      },
      size: {
        default: "h-9 px-3",
        sm: "h-8 px-2",
...
const Toggle = React.forwardRef<React.ComponentRef<typeof TogglePrimitive.Root>, ToggleProps>(
  ({ className, variant, size, tone, ...props }, ref) => (
    <TogglePrimitive.Root
      ref={ref}
      className={toggleVariants({ variant, size, tone, className })}
      {...props}
    />
  ),
);
